...
scripts/
  ...
  dev/                    — ad-hoc RE/analysis tools, not part of `npm run all`
...
knowledge/
  players/*.md            — one card per SID player family (facts + prose),
                            source of truth for reverse-engineering knowledge
  build-graph.js          — derives knowledge/graph.json (lineage edges) from the cards
  COVERAGE.md             — generated: which player families still lack a card
  playbooks/              — reusable RE procedures (e.g. disassemble-a-player.md)
  see knowledge/README.md for the full design
...
That's `fetch:all` (itself `fetch:players` → `fetch:composers` →
`fetch:csdb` → `fetch:player-media` → `fetch:hvsc-docs`) → `gaps` →
`build` chained together — 7 steps in total; see `package.json`'s
`scripts` block for the authoritative chain. Every step is independently
runnable, and every step is safe to re-run: fetch steps only hit the
network for data that isn't already cached, and `gaps`/`build` never
touch the network at all — they only read whatever's currently in
`data/`.
...
npm run fetch:player-media  # fetches CSDb release data (screenshot,
                             # credits, download link) for each curated
                             # player's csdb_id and each SIDId reference
                             # that points at a CSDb release — SKIPPED
                             # per-release if already cached under
                             # data/csdb/players.json
...
This passes `--refresh` to all five fetch scripts (composers, players,
CSDb, player media, HVSC docs), so everything gets re-requested from its
source regardless of what's on disk, then re-runs `gaps` and `build`.
...
turn that down. At 400ms per request, a cold-cache fetch of the full
seed (currently ~1,895 composers — see `data/composer-list.json`) takes
on the order of ten minutes; that's intentional.
...
- The generated page is correspondingly large now — about 10.75MB for
  ~1,900 composers and ~55,000 files (run `npm run build` and check
  `output/index.html`'s size for the current figure — `build-html.js`
  prints it at the end of every build) — including STIL.txt's per-file
