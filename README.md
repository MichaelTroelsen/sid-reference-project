...
There's no seed-generation script yet that walks the entire HVSC tree —
that would need `?folder=` calls starting from `/MUSICIANS/` and recursing
through every letter directory (A/ through Z/ and 0-9/), which is
thousands of requests at this API's rate limit. Worth building if you want
full HVSC coverage rather than a curated subset; not built here since the
seed list approach covers the composers this project actually cares about
for now.
...
- The seed list is 56 composers, not all of HVSC's ~1,872+ named
  musicians. See "Extending the seed list" above.
