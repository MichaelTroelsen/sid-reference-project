...
```json
{
  ...
  "quirks": [
    ...
    "csdb_release gap closed, confirmed genuinely absent (not a data-import gap): the live upstream sidid.nfo (fetched directly this pass, https://raw.githubusercontent.com/cadaver/sidid/master/sidid.nfo) carries only AUTHOR and COMMENT fields for the '8bitDigi/Mahoney' entry, no REFERENCE/csdb release-id field at all — matching the local data/sidid.json cache exactly. Of SIDId's 373 tags, 198 carry a `reference`; this is one of the 175 that legitimately don't, consistent with it being a personal technique fingerprint (no distributed tool, no CSDb release of its own) rather than an editor/tracker release. `csdb_release` stays `null` as a confirmed fact, not a TODO."
  ],
  "sources": [
    "SIDId sidid.nfo byTag['8bitDigi/Mahoney'] (author, comment + technique URL): https://github.com/cadaver/sidid/blob/master/sidid.nfo ; local snapshot data/sidid.json ; live-fetched and re-checked this pass at https://raw.githubusercontent.com/cadaver/sidid/master/sidid.nfo — confirmed no `reference`/csdb-release field exists for this tag",
...
independently confirmed) by an unrelated forum discussion. `csdb_release`
was this card's recorded gap; it is now confirmed `null` as a fact (not a
TODO) — a live re-fetch of upstream `sidid.nfo` this pass shows the tag
carries no `reference` field at all, consistent with a personal technique
fingerprint rather than a formally released tool. No disassembly, no
`mcp-c64` trace. `status: stub`.
