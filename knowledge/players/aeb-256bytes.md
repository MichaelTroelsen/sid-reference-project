...
```json
{
  ...
  "released": "2005 — the SID's own CSDb 'Released' field reads '2005 Carillon & Cyberiad'; earliest attested public use is the 4th-place entry in the 'Electronic' Tiny Sid C64 Music Competition release, 2005-04-24 (CSDb release id 18077), predating its reuse in 'PETSCII Ate My TinySID', 2005-06-11 (release id 18202) — confirmed via direct CSDb webservice query (scripts/lib/csdb-client.js getSidRelease(11618)), not just the SID-entry page read.",
  "status": "stub",
  "platform": "Not a distributed tool — an extreme-size-constrained custom C64 player routine + data written by Finnish composer/coder Aleksi Eeben for a size-limited demoscene competition ('Tiny Sid C64 Music Competition', run by ice00 at digilander.iol.it/ice00/tsid/tinysid per the CSDb event record), following the same pattern as the already-carded knowledge/players/agemixer-256bytes.md, knowledge/players/ice00-256bytes.md, and knowledge/players/tape-256bytes.md. 'AEB' reads as his own initials (Aleksi EEBen), not a separate person or group.",
  "csdb_release": null,
  ...
  "quirks": [
    ...
    "csdb_release IS CONFIRMED NULL, NOT JUST UNCHECKED: a direct CSDb site search for '256bytes/AEB' (https://csdb.dk/search/?seinsel=all&search=256bytes%2FAEB, fetched directly 2026-08-01) returns 'We were unable to find anything for you' across every category (releases, groups, sceners, events, BBS, SIDs) — there is no CSDb release/group page for this tag to cite, consistent with it being a one-off routine rather than a distributed tool.",
    "THE SID'S EARLIEST ATTESTED USE PREDATES THE SIBLING COMPILATION: CSDb's own release chain for csdb id 11618 (queried via scripts/lib/csdb-client.js getSidRelease) shows the tune was first released 2005-04-24 as a 4th-place entry in 'Electronic' (release id 18077), part of the 'Tiny Sid C64 Music Competition' event (organiser ice00, digilander.iol.it/ice00/tsid/tinysid) — before its later reuse in 'PETSCII Ate My TinySID' (2005-06-11, release id 18202). Both dates are 2005, so this doesn't change the recorded year, but it identifies the correct earliest release rather than resting on the later compilation.",
  ...
  "sources": [
    ...
    "CSDb webservice, scripts/lib/csdb-client.js getSidRelease(11618) queried directly — confirms DataSize 254, LoadAddr 2049 ($0801), and the UsedIn release chain: 'Electronic' (release id 18077, 2005-04-24, 4th place at the 'Tiny Sid C64 Music Competition' event, id 850, organiser ice00 at digilander.iol.it/ice00/tsid/tinysid) and 'PETSCII Ate My TinySID' (release id 18202, 2005-06-11)",
    "CSDb site search for '256bytes/AEB', https://csdb.dk/search/?seinsel=all&search=256bytes%2FAEB, fetched directly — zero results in any category, confirming no CSDb release/group page exists to populate csdb_release",
...
almost exactly (254 bytes measured) and shows the same SID's earliest release
was as a 4th-place entry (2005-04-24) in the "Tiny Sid C64 Music Competition",
before it was reused in "PETSCII Ate My TinySID" (2005-06-11) — the identical
compilation that also picked up the sibling `256bytes/Tape` and `256bytes/Ice00`
routines, though nothing suggests shared code between the three authors. A
direct CSDb site search for the tag itself returns zero results, confirming
there is no release/group page and `csdb_release` is correctly `null`, not
merely unresearched.
