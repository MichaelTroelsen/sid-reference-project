...
```json
{
  ...
  "released": "TODO: no tool-release date documented (SIDId's tag entry carries no `released`/`reference` field, and no dedicated CSDb tool/release entry exists for the technique itself). Full census of both tagged files' own CSDb SID entries: 'Channels' (csdb sid id 50330) released 2013-11-29 as the standalone one-file demo 'Channels - spadpcm1' (release id 125043); 'Boing Boom Tschak' (csdb sid id 50333) released 2014-06-08, used in the demo 'Eclectic' at Flashback 2014 (release id 131301, 1st place Mixed Demo). These are tune-attestation dates, not a tool release date.",
  "status": "stub",
  ...
  "quirks": [
    ...
    "Full census (via CSDb's XML webservice, `type=sid`, depth 2) of both tagged files, not a sample: 'Channels' (sid id 50330, load/init $0810, SID model 6581, released 2013-11-29 as the standalone one-file demo 'Channels - spadpcm1', release id 125043) and 'Boing Boom Tschak' (sid id 50333, load/init $0800, SID model 8580, released 2014-06-08, used in the demo 'Eclectic' at Flashback 2014, Sydney Australia, release id 131301, 1st place Mixed Demo). Note the release name 'Channels - spadpcm1' echoes 'spadpcm1', one of the other one-off technique productions CSDb's scener profile lists for Algorithm — a naming overlap worth flagging, but not itself evidence the two use the same routine (SIDId's own comment for THIS tag names the OxyMod/THCM 8-bit-sample technique specifically, and that reading is not second-guessed here). Load/init addresses are header metadata only, deliberately excluded from `memory`/`entry`.",
    "`csdb_release` confirmed absent, not merely unresearched: SIDId's tag entry for 'Algorithm/8bitDigi' carries only `author` and `comment` fields, no `released` or `reference` field (data/sidid.json byTag['Algorithm/8bitDigi']), and neither of the two tagged files' own CSDb SID entries points to a dedicated tool/release entry for the technique itself — only to the demo releases each tune individually appeared in."
  ],
  "sources": [
    ...
    "CSDb XML webservice, `type=sid` (full census of both tagged files' own entries for Released/UsedIn/PSID header fields): https://csdb.dk/sid/?id=50330, https://csdb.dk/sid/?id=50333",
    "CSDb release entries referenced above (used-in demos): https://csdb.dk/release/?id=125043, https://csdb.dk/release/?id=131301"
  ]
}
```
...
(full census, not a sample): 'Channels' (released 2013-11-29, standalone
one-file demo 'Channels - spadpcm1') and 'Boing Boom Tschak' (released
2014-06-08, used in 'Eclectic' at Flashback 2014, 1st place Mixed Demo) —
consistent with his pattern (per the Frodigi card) of producing multiple
personal one-off audio-technique demos. No dedicated CSDb tool/release entry
or SIDId `reference` id exists for the technique itself.
...
Algorithm's broader pattern of personal technique demos; (4) full census (not
sample) of both tagged files gives real per-tune release dates (2013-11-29
and 2014-06-08) but confirms no tool-release date or CSDb tool/release entry
exists — `released` and `csdb_release` stay `TODO`/`null` deliberately, not
from missing research.
...
the sibling `algorithm-frodigi` and `oxymod4bit-thcm` cards, one CSDb
scener-page lookup, and a full census (both tagged files, via CSDb's XML
webservice) of per-tune release dates and used-in demo records. `status: stub`.
