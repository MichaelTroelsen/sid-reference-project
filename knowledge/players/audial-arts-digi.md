...
```json
{
  ...
  "quirks": [
    ...
    "DRIFT-RECHECK 2026-08-07: re-fetched both censused files (csdb sid id 62807, 7207) plus group #752 via scripts/lib/csdb-client.js. Echodemo (62807) unchanged byte-for-byte (no UsedIn entries, same header fields). Puke-Box Main Menu (7207) gained one new UsedIn entry since the 2026-07-31 census: 'Puke-box Demo' (CSDb release id 223873, Type 'C64 Music', dated December 1991, released by scener PDB, Music credit to Rodney Balai) — a high release-ID (223873, vs. the pre-existing 5666/17969-71/21337-41 IDs), consistent with this being a page CSDb added recently rather than one this project's 2026-07-31 pass missed. Does not change the earliest-attested-use date (Dec 1991 postdates the existing March 1991 earliest use via Puke Box #02/release 5666), the `released` field, or the no-dedicated-tool-page conclusion. Group #752's page also now lists several additional group-level tune releases with high IDs (228512 'Crest Tune', 228513 'I+T Tune', 232856 'Remark Intro-tune', 237394 'Ingame Tune') and full member/founder detail (Founder: Ronny Pasch, joined 1990) not previously recorded here — none of these correspond to this card's 2 tagged files, so no field change follows. `data/sidid.json` byTag re-checked: still no entry for this exact tag. Local composer census re-run (Grep on data/composers/*.json) confirms still exactly 2 files (patrick-peters.json, rodney-balai.json) — no third file has been tagged with this signature. No field beyond `quirks`/`sources` changed; `status` and all Tier 3 fields untouched."
  ],
  "sources": [
    ...
    "DRIFT-RECHECK 2026-08-07, CSDb webservice re-fetch (scripts/lib/csdb-client.js): sid id 7207 (Puke-Box Main Menu) shows a new UsedIn entry, release id 223873 'Puke-box Demo' (C64 Music, Dec 1991, by scener PDB, music credit Rodney Balai) — https://csdb.dk/release/?id=223873 — not present in the 2026-07-31 census; sid id 62807 (Echodemo) unchanged; group id 752 unchanged in substance (BaseCountry/FoundYear/members same) — https://csdb.dk/sid/?id=7207, https://csdb.dk/sid/?id=62807, https://csdb.dk/group/?id=752; data/sidid.json and data/composers/*.json re-checked, no drift (still absent / still 2 files)"
  ]
}
```
