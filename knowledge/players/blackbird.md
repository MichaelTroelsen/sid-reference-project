...
No original disassembly was done for this card — but none was needed: the
Blackbird User's Guide (rev.2) reproduces the **complete playroutine assembly
source** in Appendix A (roughly pp.44-55 of the PDF), covering the zero-page
layout, the init/play entry-point structure, the per-frame fx/wave/filter
table reads, the note/effect byte-stream encoding, and two explicit
hard-restart workarounds for a SID "decay rate bug". Chapter 7 separately
documents the Birdcruncher export tool's `-a`/`-z` relocation options and the
runtime song-compression scheme. All Tier 3 facts in the `json` block above
are transcribed from that printed source with page citations, not inferred.
What is still genuinely unknown: the fixed base addresses of `fxtable`/
`wavetable`/`filttable`/`ins_*` (given only as external symbols in the
excerpt), pulse-table specifics, and whether the per-frame call is CIA- or
raster-triggered — none of these were stated in the material read, and are
left as `TODO` rather than guessed.
...
HVSC news archive — unchanged from the prior pass. NEW this pass: several
Tier 3 runtime facts (zero-page map, entry-point structure, note/effect
encoding, hard-restart handling, cycle budget) are now sourced directly from
the author-published playroutine source in the User's Guide's Appendix A,
satisfying this project's "a public source plainly documents a runtime fact"
allowance for `in-progress`. This has NOT been independently reassembled or
traced through `sidm2-siddump`/`mcp-c64` — that remains the bar for
`verified`, and this card does not claim to have cleared it.
