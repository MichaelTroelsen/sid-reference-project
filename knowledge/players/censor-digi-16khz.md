...
```json
{
  ...
  "quirks": [
    ...
    "CORRECTION 2026-08-07: an earlier pass of this card stated Swallow's Wonderland XII credit was 'specifically Sampling, distinct from the Code credit shared by five other members.' Re-fetching the canonical CSDb webservice record for release 120907 (via scripts/lib/csdb-client.js getRelease, cross-checked against the Demozoo production page) shows this was wrong: Swallow (handle ID 2547) holds BOTH a 'Code' credit AND a separate 'Sampling' credit on this release — the six-person Code credit list (Axis, Bob, CRT, Lavazza, Mirage, Swallow) already included him. So the 'bespoke sampling-only contribution, distinct from the shared codebase' inference does not hold as stated; Swallow was also one of the general coders on this release, which is equally (or more) consistent with him having written the digi routine himself as part of his coding work, but does not by itself distinguish 'reused shared code' from 'bespoke routine.' No other credit-list fact in the card was affected.",
    ...
    "DRIFT RECHECK 2026-08-07: all previously-cited CSDb/Lemon64/Demozoo pages re-fetched. Release 120907 (date, place, event, group record, SID header load/init addresses for all 4 files) unchanged. Lemon64 t=48956 quote unchanged ('a simple screw up by me that slipped through unnoticed', high-pitched tone fix, 16khz result). data/sidid.json still has no 'Censor_Digi/16khz' entry; data/composers/*.json census still shows only swallow.json tagging this routine (4 files, unchanged). No new CSDb tool/editor release, source archive, or edge evidence found. The only change from this pass is the Code/Sampling credit correction above; everything else in the card stands as of 2026-07-31."
  ],
  "sources": [
    ...
    "2026-08-07 drift recheck: raw CSDb webservice XML for release 120907 (scripts/lib/csdb-client.js getRelease(120907)), used to confirm/correct the Swallow Code+Sampling credit split; cross-checked against Demozoo https://demozoo.org/productions/82035/ (same Code credit list including Swallow)"
  ]
}
```
