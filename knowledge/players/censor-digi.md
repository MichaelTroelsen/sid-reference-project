...
```json
{
  ...
  "released": "TODO: no explicit release date — this is an in-house routine, not a distributed tool. Full census (2026-07-31) of all 14 tagged files' own CSDb `Released` fields (via scripts/lib/csdb-client.js, type=sid) shows earliest attested use is 1989: Wonderland IV (part 1), CSDb sid id 27909, own `Released` field '1989 Censor Design', used in Wonderland IV, CSDb release 625 (1989-11-21). This corrects an earlier spot-checked '1990' claim in this same card — Wonderland IV (1989) was missed the first time. DRIFT-RECHECK 2026-08-07: re-queried CSDb sid id 27909's `UsedIn` list directly (depth=2) and it also lists a second release, 'Dreaming' by the (unrelated) group Delusion, dated 1989-09 (day not given) — two months *before* Wonderland IV's own 1989-11-21 date, though still the same calendar year, so the '1989' earliest-attested-year conclusion is unchanged. Not asserted as newly-added CSDb data (can't confirm it wasn't simply un-enumerated by the original single-release-per-file census); recorded here for completeness. https://csdb.dk/release/?id=41317",
  "status": "stub",
  ...
  "quirks": [
    ...
    "DRIFT-RECHECK, 2026-08-07 (stub sample recheck, no fresh gap-fill): re-verified all previously-cited sources for new information since 2026-07-31. Local dataset unchanged (data/composers/swallow.json + tomas-danko.json still show exactly 8 Censor_Digi_1 + 6 Censor_Digi_2 = 14 files, 2 composers — grep re-run with explicit *.json glob). deepsid_dl/sidid.nfo re-fetched fresh from GitHub raw and byte-searched directly (not via the WebFetch tool's summarizing sub-model, which on a first pass hallucinated a 'Censor_Digi_1' entry bearing the OxyMod/THCM comment that actually belongs to the sibling Censor_8bit_Digi_1 — confirmed a hallucination by re-fetching the raw .nfo text and running indexOf('Censor_Digi_1') = -1 directly): still no Censor_Digi_1/_2 entry exists, card's claim stands. CSDb group 2310 (Censor Design) and scener 2547 (Swallow) re-checked: founding date (1989-06-28) and country (Sweden) unchanged; no new 'Censor Digi' tool/editor release appeared on the group's page (2026 releases are SpearHunter/graphics, Transfluid Tut-Tut/music, 50 Plasmas for Free/demo — none are a digi tool). Lemon64 thread (viewtopic.php?p=164881) re-read: TMR's 4-bit quote unchanged, no later post in that thread revisits Censor's routine. One new-to-this-card CSDb detail found on direct re-query (not previously enumerated): sid 27909's UsedIn list includes a second release beyond the one cited (see corrected `released` field note) — otherwise several other tagged files also have additional UsedIn reuse entries (diskmag reprints, compilations e.g. 'Publication #8', 'Contact Note', '1 Year in Wonderland') not individually enumerated in the original census, none of which change any composer/date/platform conclusion already recorded. No other drift found. `status` stays `stub`; no Tier 3 field touched."
  ],
  "sources": [
    ...
    "DRIFT-RECHECK 2026-08-07: deepsid_dl/sidid.nfo re-fetched fresh (raw.githubusercontent.com/cadaver/sidid/master/sidid.nfo, latin1-decoded) and byte-searched directly for 'Censor_Digi_1'/'Censor_Digi_2' — confirmed absent, no entry; CSDb webservice type=sid id=27909 depth=2 re-queried directly (scripts/lib/csdb-client.js), showing an additional UsedIn release not previously cited: 'Dreaming' by Delusion, https://csdb.dk/release/?id=41317 (1989-09, predates but same year as Wonderland IV's 1989-11-21)"
  ]
}
```
