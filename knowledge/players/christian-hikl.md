...
```json
{
  ...
  "quirks": [
    ...
    "csdb_release GAP CLOSED, RESULT NEGATIVE: no CSDb release id exists for this tag, confirmed by two independent CSDb lookups (2026-08-01): a name search for 'Christian Hikl' (csdb.dk/search/?seinsel=all&search=Christian+Hikl) returns exactly the same 9 SID entries as the earlier 'Hikl' search — no release, group, or tool entry — and the SID page for the tagged Jump'n_Jive.sid (csdb.dk/sid/?id=52273) links back only to HVSC/DeepSID, no release id. Consistent with the card's own finding that this is a per-tune routine, not a published tool, so there is nothing for csdb_release to point at. `csdb_release: null` is confirmed correct, not merely unresearched. That same SID page independently corroborates the disassembly-derived load/play addresses ($9060 / $906B)."
  ],
  "sources": [
    ...
    "CSDb search (returns only his 9 SIDs — no scener, no groups): https://csdb.dk/search/?seinsel=all&search=Hikl · full-name re-check, same negative result: https://csdb.dk/search/?seinsel=all&search=Christian+Hikl",
    "CSDb SID page for the tagged Jump'n_Jive.sid, confirming no linked release id and corroborating load/play addresses: https://csdb.dk/sid/?id=52273",
