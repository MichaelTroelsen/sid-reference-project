...
```json
{
  ...
  "quirks": [
    ...
    "CLUSTER NOTE (not asserted as an edge — no direct evidence found): the same author, Pawel Kulikowski (Comer), also has two other distinctly-tagged routines in this dataset, Comer/NMI_Sample_5 (CSDb release 101599, credited to 'Comer of Taboo', group Taboo) and Comer/Digi (no CSDb reference in SIDId at all). Sample Studio and NMI_Sample_5 are separate CSDb release IDs with different credited groups (individual vs. Taboo) and no CSDb note, forum post, or SIDId comment found linking them as versions of each other or as tool+successor. Composer overlap alone (Black and Comer use both Sample_Studio and NMI_Sample_5) is NOT treated as lineage evidence per this project's rules — it just reflects a small clique of Polish composers who used whichever of Comer's tools was current. Treat as three separate, unverified routines by the same author unless a byte-signature or source comparison says otherwise. A FOURTH Comer tool, 'Stereo Sample Studio V1' (CSDb release 121424, Code: Comer of Taboo, per sibling card comer-nmi-sample-5.md's research), exists but is not SIDId-tagged and matches no file in this dataset — noted for completeness only, not part of this card's subject.",
    "GAP-FILL PASS (2026-08-01): re-verified CSDb release #101704 live via the XML webservice (scripts/lib/csdb-client.js, type=release, id=101704) — Name 'Sample Studio', Type 'C64 Tool', ReleaseYear 1993, ReleasedBy Comer (handle id 7572, Poland, alt handle Cyklon) all match the card exactly. The webservice response also surfaces a CSDb production-note comment (by JCH/Chordian, dated 25.04.2019) not previously cited here: \"Hitting 'O' for the organizer screen reveals that it was made in 1993,\" with a screenshot of the in-program organizer screen — independent corroboration of the 1993 release year from the tool's own UI, beyond the bare CSDb release-year field. Full file census re-run against data/composers/*.json (glob *.json) and reconfirms exactly 17 files across the same 6 composers (blackman.json:3, black.json:4, comer.json:7, data.json:1, joan.json:1, surgeon.json:1) — no drift, no missed cluster."
  ],
  "sources": [
    ...
    "CSDb webservice re-check, 2026-08-01, type=release&id=101704&depth=2 (confirms Name/Type/ReleaseYear/ReleasedBy exactly, plus a JCH production-note comment dated 25.04.2019 stating the in-program organizer screen shows it was made in 1993): https://csdb.dk/webservice/?type=release&id=101704&depth=2",
    ...
    "CSDb release #121424 'Stereo Sample Studio V1' (Code: Comer of Taboo; not SIDId-tagged, no file in this dataset uses it) — per sibling card knowledge/players/comer-nmi-sample-5.md's research, cited here only for cluster completeness: https://csdb.dk/release/?id=121424",
    ...
    "Local dataset: 17 files tagged Comer/Sample_Studio across 6 composers (Black, Blackman, Comer, Data, Joan, Surgeon) — grep across data/composers/*.json (black.json, blackman.json, comer.json, data.json, joan.json, surgeon.json); all 6 real names cross-checked as Polish handles from each composer's own author string. Re-censused 2026-08-01, exact match, no drift.",
...
A gap-fill pass (2026-08-01) independently re-queried CSDb release #101704 live
via the XML webservice and re-ran the full file census against
`data/composers/*.json`; both reconfirm the existing card exactly (same
Name/Type/ReleaseYear/ReleasedBy, same 17 files/6 composers), and surfaced one
new corroborating detail (a JCH production note dated 25.04.2019, citing the
in-program organizer screen as showing "made in 1993"), now added to `quirks`/
`sources`. `status: stub`.
