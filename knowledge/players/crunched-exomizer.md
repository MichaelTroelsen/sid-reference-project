...
```json
{
  ...
  "quirks": [
    ...
    "CENSUS RE-CONFIRMED (2026-08-01): re-grepped all of data/composers/*.json for the exact tag; still exactly 2 hits, both in the files already named above (Cadaver's 'Metal_Warrior_4_goatpack.sid', csdb_id 5452; Edward Hobbs's 'Castles_of_Doctor_Creep.sid', csdb_id 1240). No data/sidid.json byTag key matches /exomizer/i at all (checked case-insensitively) — corroborates this was never a SIDId-catalogued player signature.",
    "SUGGESTIVE BUT NOT ASSERTED AS AN EDGE: the Cadaver file's own name is 'Metal Warrior 4 (goatpack)', and a sibling entry for the same tune in the same composer file, 'Metal Warrior 4 (preview)', is tagged 'GoatTracker_V1.x' (data/composers/cadaver.json). This suggests (but does not prove, since goatpack is GoatTracker's own linker/relocator tool, not evidence of the runtime player used at play time) that the underlying music in at least this one Exomizer-crunched file is GoatTracker-authored — the per-file investigation the card already says would be needed, not yet done, and not promoted to an `edges` claim here for lack of direct proof.",
  ...
  "sources": [
    ...
    "2026-08-01 gap-fill pass: re-verified authors/csdb_release gaps directly — grepped data/composers/*.json for the exact tag (2 hits, matching prior census) and data/sidid.json byTag keys against /exomizer/i (0 matches). Both fields remain legitimately empty/null: there is no SIDId entry and no dated CSDb release to cite, because this tag names a packer, not a released tool"
  ]
}
```
