...
```json
{
  ...
  "quirks": [
    ...
    "'Dutch USA-Team' is ONE group, not two ('Dutch Team' + 'USA Team'): verified directly via CSDb's group record (id 1392, AKA list 'USA-Team, Dutch Unlimited Software Association Team, The Great USA-Team, The Dutch USA-Team', founded Netherlands 1986). Its own Trivia field #1 explicitly disambiguates from an unrelated German group sharing the 'USA-TEAM' name ('Not to be confused with the German group with the same USA-TEAM name ... utterly silly puberty territorial pissings ... we actually met with the German team at Danish Gold and everything was okay'), and #4 notes the Rockmonitor editors were 'modified versions of Sound Monitor by Chris Huelsbeck' (context for the sibling rockmonitor.md card, not evidence of any Pro-Drum code reuse). Handle 'MC' (Handle ID 4074) independently confirmed via CSDb scener records to resolve to current handle 'Marco Swagerman' (Handle ID 6151), Netherlands, functions Coder/Musician, member of Dutch USA-Team — matching this card's author attribution exactly."
  ],
  "sources": [
    ...
    "Round-27 re-verification (2026-08-01): re-censused all 4 tagged composer files directly against data/composers/*.json (commander.json x5, ice-t.json x2, pilot.json x1, pvcf.json x1 = 9, exact match) and re-fetched CSDb releases 20672/33854/153558/74041 plus group 1392 and scener records for handles MC/Marco Swagerman via scripts/lib/csdb-client.js — every prior fact confirmed live, no corrections needed."
  ]
}
```
