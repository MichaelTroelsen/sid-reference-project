...
```json
{
  ...
  "memory": {
    "load_address": "Varies per file (relocatable player embedded per tune). $1000 in the public ACME reconstruction of 'Sound Routine f 4.1.0' (Adam Bulka, 1988) — https://github.com/realdmx/c64_6581_sid_players/blob/master/Bulka_Adam_FAME/Bulka_Adam_SoundRoutine_f_4.1.0.asm — versus $7000 on the unrelated-version 'My Instinct' SID per its PSID header (CSDb: https://csdb.dk/sid/?id=12043). Not a fixed family constant.",
  ...
  "entry": {
    "init": "Label 'musicset0' in the reconstructed source (assembled at $1000 there): clears $D400-$D418 twice (silences all 3 voices + filter), then loads the 3 voices' order-list pointers ('trackslohi' table, 6 bytes per song entry) selected by the PSID song number, and a per-song tempo byte from 'speedtab' self-modified into the 'speed2' compare. Source: https://github.com/realdmx/c64_6581_sid_players/blob/master/Bulka_Adam_FAME/Bulka_Adam_SoundRoutine_f_4.1.0.asm — read directly off the labelled source, not independently re-traced.",
    "play": "Label 'music': loops X=2..0 over the 3 SID voices, one pass per voice per call, each voice fetching its current order-list step and dispatching on the command byte (see effects.encoding). Same source as init. Call rate/interrupt setup (raster vs CIA, 1x vs multispeed) is NOT present in this fragment — it is a bare init/play pair meant to be driven by whichever game/intro embeds it, so left TODO rather than guessed."
  },
...
encoding — enough to move `status` from `stub` to `in-progress`.
