...
```json
{
  ...
  "released": "2009-09-21 (technique debuted on Maniacs of Noise's demo/production 'Echoes', CSDb release 82788, code/music/idea credited to GT = Geir Tjelta, containing SID 42404); 'Echofied 6581' showcase followed 2010-05-02, CSDb release 91173",
  "status": "stub",
  ...
  "quirks": [
    ...
    "Composer concentration (Tier 1, from data/composers/*.json): 10 files across 5 composers -- Geir Tjelta himself 4 (40%: Artillery (credits) csdb_id 44386, Echoes csdb_id 42404, Echofied 6581 csdb_id 42849, Oro Incenso csdb_id 42848), Glenn Gallefoss/GRG 2 (csdb_id 59681, 59682), Knutsi Aleksi 2 (csdb_id 60879, 60878), DRAX 1 (csdb_id 43184), Et1999cc 1 (csdb_id 61058). Filenames literally flag usage of the routine ('Sub Hunter Highscore (echoed)', 'Buzzer (echoed)', 'Autumnness (echofied)', 'Echoes', 'Echofied 6581') -- a routine shared within a small Norwegian-scene circle around Tjelta, not a widely-adopted tool. Matches the project's 'few composers = personal/small-scene routine' heuristic. Census of all 10 tagged files (not a sample) confirms this exact 4/2/2/1/1 split -- see knowledge/EXTRACTION-TEMPLATE.md 'Census every tagged file' rule.",
    "csdb_release gap re-checked (2026-07-31): confirmed still absent. CSDb webservice queried directly for both known SID entries' UsedIn releases (82788 'Echoes' by Maniacs of Noise, 2009-09-21, code/music/idea: GT; and 91173 'Echofied 6581', 2010-05-02) -- both are C64 Music demo/production releases that USE the routine, not a tool-release page FOR a player/editor named 'Echo'. No SIDId `reference` exists to source `csdb_release` from (see prior quirk on sidid.nfo absence), and no separate CSDb release entry for an 'Echo' tool/player was found via web search of csdb.dk. `csdb_release: null` is confirmed correct, not merely unresearched."
  ],
  "sources": [
    "CSDb SID entry 42404, 'Echoes' by Geir Tjelta (2009, Maniacs of Noise) -- earliest known use: https://csdb.dk/sid/?id=42404",
    "CSDb release 82788, 'Echoes' by Maniacs of Noise, exact date 2009-09-21 (queried via scripts/lib/csdb-client.js webservice, type=release id=82788) -- code/music/idea credited to handle GT (Geir Tjelta), contains SID 42404; the demo that debuted the technique: https://csdb.dk/release/?id=82788",
    "CSDb release 91173, 'Echofied 6581' by Maniacs of Noise, exact date 2010-05-02 (confirmed via csdb-client.js webservice query) (code: GT; music: Drax, GT, Jeroen Tel, Laxity) -- demonstration/showcase release, includes the $BF00-$BFFF lookup-table comment (unverified): https://csdb.dk/release/?id=91173",
...
channel" of subtle echo. It debuted on Maniacs of Noise's demo "Echoes"
(CSDb release 82788, 2009-09-21) and was showcased again on "Echofied 6581"
(CSDb release 91173, 2010-05-02); it sits alongside Tjelta's other, better
...
See the `sources` array -- CSDb (SID entry 42404, releases 82788 and 91173,
webservice-queried directly for exact dates via `scripts/lib/csdb-client.js`),
the CHIPFLIP scene-technique writeup, a confirmed absence in SIDId's
sidid.nfo, and the local per-composer dataset. The recorded gap
(`csdb_release`) was re-checked on 2026-07-31: it stays `null` on purpose --
both known CSDb releases are demo/production entries that *use* the routine,
not a tool-release page *for* an "Echo" player/editor, and no such page was
found on csdb.dk.
