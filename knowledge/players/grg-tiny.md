...
project's own `COVERAGE.md` grouping and per-composer dataset, an HVSC news
changelog entry documenting `GRG Tiny4` as a distinct signature (with no
separate `GRG` entry found anywhere in it), two
CSDb SID entries used to bound the usage timeline, and Gallefoss's scener
identity. The related `sidduzzit.md`/`digitalizer.md` cards are cited only as
