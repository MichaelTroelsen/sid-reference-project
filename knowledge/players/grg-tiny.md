...
```json
{
  ...
  "released": "TODO: no fixed release date found — this is an iterated personal routine, not a dated publication. Re-checked 2026-07-31 via CSDb webservice sid-entry lookups (not just filenames) for a representative file per tag: GRG_tiny_1 spans CSDb-dated 2002 ('Final Fantasy', sid id 2737) to 2011 ('N0s Intro', sid id 44841); GRG_tiny_2 spans 2011 ('Hysteria Pimped', sid id 45599) to at least 2012 ('Nomen Nescio', sid id 46633); GRG_tiny_3 spans 2013 ('Into the Abyss', sid id 48785) to 2016 ('Beyond the Gates of Dawn (v1)', sid id 53027); GRG_tiny_4 spans 2018 ('Crystal Gazer (tune 2)', sid id 55900) to 2025 ('N0s Intro 84', sid id 65178). This corrects the prior estimate, which took the tune title 'Lightforce 2006' at face value as the earliest date; CSDb's actual Released field puts GRG_tiny_1 two years earlier (2002), and the four tag-versions' date ranges overlap at their boundaries (2011, 2013, 2016, 2018) rather than cleanly succeeding each other.",
  "status": "stub",
  ...
  "quirks": [
    ...
    "Re-research pass, 2026-07-31 (focused on `released`/`csdb_release` only, no Tier 3 work): queried CSDb's XML webservice directly (csdb-client.js's underlying endpoint, via curl since Bash-tool `node`'s own `fetch` hung/timed out against csdb.dk in this sandbox — `curl` worked) for scener id 8098 (GRG/6R6/Shark/Perts, all one CSDb person record) at depth=2, extracting all 952 unique `<Name>` release-name strings tied to him: no release named 'GRG Tiny' or similar exists (two near-miss matches, 'Tiny Sid 2 C64 Music Competition' and 'Unofficial Tiny SID Compo 2022', are unrelated compos he did NOT organize/enter as a tool author). Confirms the earlier scener-page finding at a different depth/method. Also fetched Demozoo's full production list for this scener (425+ entries, 1991-2026) via WebFetch: the only C64 'Tool' category credits are SID Duzz' It V1.3/V1.5 (already known, see sources) and a previously-unrecorded 'Blues Muz' Player V1.0' (June 1994, demozoo.org/productions/369467/) credited to Gallefoss under the alias 'Shark', not GRG — noted here for completeness but NOT treated as GRG Tiny under a different name (different alias, different era, no source connects the two), so not added as an alias or edge.",
    "`released` field corrected this pass (2026-07-31): the prior estimate's lower bound (2006) came from taking the tune title 'Lightforce 2006' at face value; checking CSDb's actual per-file `<Released>` metadata (via `type=sid` webservice calls) for one representative file per GRG_tiny_1..4 tag shows real usage starts earlier, in 2002 ('Final Fantasy', GRG_tiny_1, CSDb sid id 2737), and that the four versions' documented date ranges overlap at their boundaries (2011, 2013, 2016, 2018) rather than cleanly succeeding one another — consistent with the versions being maintained/reused in parallel rather than one strictly retiring the last.",
    "`csdb_release` confirmed null this pass, precisely: CSDb's `UsedIn` field on the GRG_tiny_4 sid entry (id 65178, 'N0s Intro 84') points to release id 256075, 'Nostalgia Intro 84' (a C64 Crack Intro, released 2025-09-16, https://csdb.dk/release/?id=256075) — but that is the *demo the tune was used in*, not a release of the GRG Tiny player/tool itself, and CSDb has no separate release entry for 'GRG Tiny' as a program. Recording this distinction explicitly rather than writing 256075 into `csdb_release`, per this project's release-vs-sid-entry namespace rule."
  ],
  "sources": [
    ...
    "Re-research pass 2026-07-31: CSDb webservice, scener id 8098, depth=2 (952 unique release names across all his handles/groups; queried directly via curl against scripts/lib/csdb-client.js's endpoint since node fetch hung in this sandbox): https://csdb.dk/webservice/?type=scener&id=8098&depth=2 — no 'GRG Tiny' release found",
    "Re-research pass 2026-07-31: CSDb webservice per-file sid lookups used to redate the `released` span: id=2737 'Final Fantasy' (2002, GRG_tiny_1), id=44841 'N0s Intro' (2011, GRG_tiny_1), id=45599 'Hysteria Pimped' (2011, GRG_tiny_2), id=46633 'Nomen Nescio' (2012, GRG_tiny_2), id=48785 'Into the Abyss' (2013, GRG_tiny_3), id=53027 'Beyond the Gates of Dawn (v1)' (2016, GRG_tiny_3), id=55900 'Crystal Gazer (tune 2)' (2018, GRG_tiny_4), id=65178 'N0s Intro 84' (2025, GRG_tiny_4) — e.g. https://csdb.dk/webservice/?type=sid&id=2737&depth=1",
    "Re-research pass 2026-07-31: CSDb sid entry 65178's `UsedIn` field, showing the tune was used in release id 256075 'Nostalgia Intro 84' (C64 Crack Intro, 2025-09-16) — the demo, not a GRG Tiny tool release: https://csdb.dk/release/?id=256075",
    "Re-research pass 2026-07-31: Demozoo full production list for scener 948 (425+ entries, 1991-2026), fetched via WebFetch — only C64 'Tool' credits are SID Duzz' It V1.3/V1.5 and 'Blues Muz' Player V1.0' (June 1994, alias 'Shark', not GRG): https://demozoo.org/sceners/948/ ; https://demozoo.org/productions/369467/",
    "Re-research pass 2026-07-31: Tier 1 re-check, data/sidid.json byTag confirms GRG/GRG_tiny_1..4 entries carry only an `author` field, no `released`/`reference`/`comment` — nothing to derive a date from at that level"
  ]
}
```
...
two decades (CSDb per-file dates span 2002 to 2025, corrected from an earlier
2006 estimate — see quirks) rather than a released tool.
