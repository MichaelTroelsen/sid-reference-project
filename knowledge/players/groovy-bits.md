...
None. No public source or disassembly of the Groovy Bits/GB-Composer replay routine was located. The one concrete lead is CSDb release 27462's docs file (`GB_Composer_2_6_plus_docs.prg`) — likely user-facing editor instructions, not a technical spec, but unread; a future pass could fetch it, and separately disassemble a representative `Groovy_Bits`-tagged `.sid` from its PSID header (init/play addresses) and trace it through `sidm2-siddump` — the only route to real memory-map/format facts.
