...
```json
{
  ...
  "quirks": [
    ...
    "UPDATE 2026-08-07 (drift recheck): CSDb's 'Groovy Bits (Sweden)' group page (https://csdb.dk/group/?id=337) lists three additional catalogued V1.x tool releases not previously cited on this card, all Karl XII of Agile solo code credits (no Roy co-credit shown): 'Groovy Beat V1.8' (CSDb release 38997, 1989, alt title 'Ed V1.8+++'), 'Groovy Music V1.5' (CSDb release 150411, 1989), and 'Groovy Music V1.6' (CSDb release 121370, 1989, alt titles 'Groovy Bits Music Version 1.6' / 'Groovy Beat 1.6'). This extends the documented V1.x point-release sequence beyond the V1.0/V1.1 pair already on this card, and may explain the 'alternate title Groovy Music v1.6' noted on the V1.0 page (release 10740) as a separate re-dump/re-label rather than a single unique artifact — not independently resolved. It is not established whether these three release pages are new to CSDb since the 2026-07-23 pass or were simply not surfaced by that pass's searches; either way, no new co-author, lineage, or runtime fact resulted — all three are consistent with the existing `authors` field. The four previously-cited release pages (10740, 121513, 27462, 184076) were re-fetched and show no changed credits, dates, or corrected release types; all visible comments on them are pre-2020 and were already known."
  ],
  "sources": [
    ...
    "Local dataset: 64 files tagged 'Groovy_Bits' across 9 composers, verified by grep/aggregation of data/composers/*.json. UPDATE 2026-08-07: knowledge/COVERAGE.md no longer lists this family by name (it now only lists the 1 remaining uncarded family project-wide, since Groovy Bits already has a card) — the earlier 'line 26' citation is stale as a locator, though the underlying figure remains independently confirmed straight from data/composers/*.json.",
    ...
    "Re-verified again on 2026-08-07 (drift recheck) directly against each composer's `folder` array in data/composers/*.json (glob \"*.json\"), matching on the raw player tag string: identical result, 64 files across the same 9 composers with the same per-composer breakdown. No drift in the local dataset.",
    "CSDb group page 'Groovy Bits (Sweden)': https://csdb.dk/group/?id=337 (lists additional V1.x releases 38997, 150411, 121370 — see quirks)",
    "CSDb release 'Groovy Beat V1.8' (Groovy Bits, 1989; code Karl XII of Agile): https://csdb.dk/release/?id=38997",
    "CSDb release 'Groovy Music V1.5' (Groovy Bits, 1989; code/music Karl XII of Agile): https://csdb.dk/release/?id=150411",
    "CSDb release 'Groovy Music V1.6' (Groovy Bits, 1989; code Karl XII of Agile): https://csdb.dk/release/?id=121370"
  ]
}
```
