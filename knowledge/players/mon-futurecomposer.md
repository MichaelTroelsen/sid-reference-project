...
<!--
  ...
  the same style of evidence. Four other "/TTWII", "/Bantam",
...
```json
{
  ...
  "aliases": ["MoN/FutureComposer", "MoN/FutureComposer/RWE", "MoN/FutureComposer/Cyb2"],
  ...
  "quirks": [
    ...
    "Four OTHER sibling raw tags still share the 'MoN/FutureComposer/' prefix with a suffix and remain UNMERGED, NOT claimed as aliases of this card — TTWII (18 files), Bantam (14), Deenen_Digi (11), JTS (11), per knowledge/COVERAGE.md. Each is tracked as its own separate uncarded family; do not assume the RWE/Cyb2 merges above extend to these without independently checking each one's own composer/file evidence and SIDId-signature-absence first.",
  ...
  "sources": [
    ...
    "knowledge/COVERAGE.md — the four other sibling '/TTWII', '/Bantam', '/Deenen_Digi', '/JTS' tags remain uncarded and unmerged; the bare 'MoN/FutureComposer' tag and the RWE/Cyb2 sub-tags no longer appear in COVERAGE.md's uncarded list once it is regenerated after this card's alias merges",
    "Local aggregate over data/composers/*.json: bare tag 97 files/24 composers; '/RWE' tag 45 files/3 composers (TC 42, Francois Prijt 2, Charles Deenen 1); '/Cyb2' tag 38 files/9 composers (Markus Siebold 10, Jeroen Tel 7, Charles Deenen 6, Reyn Ouwehand 5, Mad_Donne_Marcel 4, Ace64 2, Drumbeat 2, No-XS 1, SounDemon 1); combined 180 files/30 composers (TC 46, Jeroen Tel 26, Markus Schneider 15, Charles Deenen 13, SMC 13, Markus Siebold 11 [1 bare + 10 Cyb2], Barry Leitch 6, Mad_Donne_Marcel 6, and 22 more with 1-5 each)",
    ...
    "data/composers/charles-deenen.json — confirms 'RWE_Intro.sid' (Charles Deenen's own file) carries the 'MoN/FutureComposer/RWE' tag, csdb_id 10647",
    ...
    "VGMPF wiki 'Future Composer' article (https://www.vgmpf.com/Wiki/index.php?title=Future_Composer, checked 2026-07-16) — corroborates the FCS/MoN origin story and the 1988 threatened-lawsuit history already in future-composer.md; no Cyb2-specific detail",
    ...
    "knowledge/players/rockmonitor.md — precedent for merging a SIDId-entry-less sub-tag (RockMon3h) into a parent card by composer-concentration + no-independent-signature evidence, same reasoning applied here for RWE and Cyb2",
...
MoN/FutureComposer is a Player-ID/SIDId raw tag covering (as of the RWE and
Cyb2 merges) 180 files across 30 composers in this collection — 97 files/24
composers under the bare `MoN/FutureComposer` tag, plus 45 files/3 composers
under `MoN/FutureComposer/RWE` and 38 files/9 composers under
`MoN/FutureComposer/Cyb2`, both folded in as aliases (see the JSON header
comment and quirks for the evidence). Per SIDId's own entry it identifies
the same tool already covered by [`future-composer.md`](future-composer.md)
— the **FCS/Finnish Gold** editor (credited to Juha Granberg) wrapping a
**Maniacs of Noise** replay driver (Charles Deenen), first released as
"Future Composer V1.0" on 20 Jun 1988 (CSDb release id 10604) — but detected
here under *different* raw signatures than the
`FutureComposer_V1.0`/`V3.x`/`V4_Packed`/`FC` tags that card's aliases
already claim. The two families are kept as separate cards because they are
separate, independently-tracked tag strings in this project's coverage
accounting, and no byte-level comparison has been done to confirm they are
identical binaries. Composer usage is broadly spread across the combined 30
composers (top 5 are 62% of the total; TC's 26% — entirely from RWE — is the
single biggest skew), consistent with FutureComposer's status as a
...
The working theory for why DeepSID's Player-ID scanner produced distinct
`/RWE` and `/Cyb2` (and other) suffix tags at all is that each names a
byte-level signature variant after its own fingerprint/reference tune
(`RWE_Intro.sid`, Jeroen Tel's `Cybernoid II`) rather than after a sub-group
or cracker credit — see the quirks for the evidence trail.
...
resolved; and (3) **`MoN/FutureComposer/RWE` and `MoN/FutureComposer/Cyb2`
were both merged into this card's aliases (2026-07-16)** on the same style
of evidence — no independent SIDId/upstream `sidid.nfo` signature, plus
authorship overlap with MoN's own founders (Deenen/Tel) — but what "RWE"
denotes remains unresolved, while "Cyb2" is reasonably explained (not
proven) as short for "Cybernoid II", the reference tune among its files. See
the dedicated quirk entries for both. Four sibling `/TTWII`, `/Bantam`,
`/Deenen_Digi`, `/JTS` tags remain separate and unmerged.
...
against a file carrying this `MoN/FutureComposer` tag (or its RWE/Cyb2
...
(https://csdb.dk/sid/?id=10647) states a load address of **$1000**, while
its page for the Cyb2-tagged `Cybernoid II` (https://csdb.dk/sid/?id=28140)
states load **$A600** / init **$A600** / play **$A603** — three different
values across two files of this same merged family (and different again
...
The RWE and Cyb2 alias merges are both identity/provenance-level research
(tag signature absence + authorship/date evidence), not a runtime
verification — neither changes the card's `status`.
...
release page, the CSDb SID entries for "RWE Intro" (id 10647) and
"Cybernoid II" (id 28140), the VGMPF Future Composer wiki article, the
`rockmonitor.md` merge precedent, and the sibling
