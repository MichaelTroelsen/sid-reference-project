...
<!--
  ...
  fingerprint-tune naming pattern). Two other "/Deenen_Digi", "/JTS" sibling
  tags remain separate, unmerged uncarded families — do not assume they
  belong here too without the same per-tag verification.
-->
...
```json
{
  ...
  "aliases": ["MoN/FutureComposer", "MoN/FutureComposer/RWE", "MoN/FutureComposer/Cyb2", "MoN/FutureComposer/TTWII", "MoN/FutureComposer/Bantam"],
  ...
  "quirks": [
    ...
    "Composer concentration with Bantam also merged in (final, all five aliases combined): 212 files across 34 composers. TC:46 (21.7%, down from 23.2% — Bantam adds no TC files, so the share shrinks as the denominator grows, the same effect TTWII had), Jeroen Tel:32 (15.1%, up from 26 — Bantam is Tel's biggest single contribution of any sub-tag), Charles Deenen:15 (7.1%), Markus Schneider:15 (7.1%), SMC:13 (6.1%), Defbeat:11 (5.2%), Markus Siebold:11 (5.2%), Mozes Emotional:6 (2.8%, entirely from Bantam, new to the family), and 26 more composers with 1-6 files each. Overall read is unchanged and, if anything, strengthened: 34 composers, no single one over a quarter of the total, still consistent with a genuinely-adopted public tool rather than a personal routine.",
    "Two OTHER sibling raw tags still share the 'MoN/FutureComposer/' prefix with a suffix and remain UNMERGED, NOT claimed as aliases of this card — Deenen_Digi (11 files) and JTS (11 files), per knowledge/COVERAGE.md. Each is tracked as its own separate uncarded family; do not assume the RWE/Cyb2/TTWII/Bantam merges above extend to these without independently checking each one's own composer/file evidence and SIDId-signature-absence first. Out of scope for the 2026-07-17 Bantam research pass by explicit instruction.",
  ...
  "sources": [
    ...
    "knowledge/COVERAGE.md — the two remaining sibling '/Deenen_Digi', '/JTS' tags remain uncarded and unmerged (out of scope for the 2026-07-17 pass); the bare 'MoN/FutureComposer' tag and the RWE/Cyb2/TTWII/Bantam sub-tags no longer appear in COVERAGE.md's uncarded list once it is regenerated after this card's alias merges",
    "Local aggregate over data/composers/*.json: bare tag 97 files/24 composers; '/RWE' tag 45 files/3 composers (TC 42, Francois Prijt 2, Charles Deenen 1); '/Cyb2' tag 38 files/9 composers (Markus Siebold 10, Jeroen Tel 7, Charles Deenen 6, Reyn Ouwehand 5, Mad_Donne_Marcel 4, Ace64 2, Drumbeat 2, No-XS 1, SounDemon 1); '/TTWII' tag 18 files/5 composers (Defbeat 11, Francois Prijt 3, Rock Captain 2, Charles Deenen 1, Commander 1); '/Bantam' tag 14 files/4 composers (Jeroen Tel 6, Mozes Emotional 6, Charles Deenen 1, Red_Duijckaerts_Roger 1); combined (all five) 212 files/34 composers (TC 46, Jeroen Tel 32, Charles Deenen 15, Markus Schneider 15, SMC 13, Defbeat 11, Markus Siebold 11, Barry Leitch 6, Mad_Donne_Marcel 6, Mozes Emotional 6, and 24 more with 1-5 each)",
    "data/composers/jeroen-tel.json — confirms Noisy_Pillars_tune_1.sid's own player tag in this dataset is 'MoN/FutureComposer'; also confirms Jeroen Tel's own 'Cybernoid II' file carries the 'MoN/FutureComposer/Cyb2' tag; also confirms Jeroen Tel's own 'Bantam.sid' carries the 'MoN/FutureComposer/Bantam' tag, csdb_id 28085",
    ...
    "knowledge/players/rockmonitor.md — precedent for merging a SIDId-entry-less sub-tag (RockMon3h) into a parent card by composer-concentration + no-independent-signature evidence, same reasoning applied here for RWE, Cyb2, TTWII, and Bantam",
...
Cyb2, TTWII, and Bantam merges) 212 files across 34 composers in this
...
`MoN/FutureComposer/TTWII`, and 14 files/4 composers under
`MoN/FutureComposer/Bantam`, all four folded in as aliases (see the JSON
...
combined 34 composers (TC's 21.7% — entirely from RWE — is the single
biggest skew, and its share keeps shrinking with each further merge since
none of RWE/Cyb2/TTWII/Bantam except RWE itself contributes any TC files),
consistent with FutureComposer's status as a genuinely adopted public tool
rather than a personal routine — matching the same conclusion
`future-composer.md` reaches from its own, larger tag set. The working
theory for why DeepSID's Player-ID scanner produced distinct `/RWE`,
`/Cyb2`, `/TTWII`, and `/Bantam` (and other) suffix tags at all is that each
names a byte-level signature variant after its own fingerprint/reference
tune (`RWE_Intro.sid`, Jeroen Tel's `Cybernoid II`, Charles Deenen's own
"That's the Way It Is (intro)", Jeroen Tel's own `Bantam.sid`) rather than
after a sub-group or cracker credit — see the quirks for the evidence trail.
...
`MoN/FutureComposer/TTWII`, and `MoN/FutureComposer/Bantam` were all merged
into this card's aliases (RWE/Cyb2/TTWII on 2026-07-16, Bantam on
2026-07-17)** on the same style of evidence — no independent SIDId/upstream
`sidid.nfo` signature, plus authorship overlap with MoN's own founders
(Deenen/Tel) — but what "RWE" denotes remains unresolved, while "Cyb2",
"TTWII", and "Bantam" are all reasonably explained (not proven beyond the
naming pattern) as short for, or identical to, their own reference tunes
("Cybernoid II", "That's the Way It Is", "Bantam.sid" itself). TTWII's own
...
what matters is that none shifts the COMBINED family's read away from
"broadly adopted tool". See the dedicated quirk entries for all four
merges. Two sibling `/Deenen_Digi`, `/JTS` tags remain separate and
unmerged (out of scope for the 2026-07-17 pass).
...
RWE/Cyb2/TTWII/Bantam aliases), so it is not copied into this card's
...
states load **$1500** / init **$1500** / play **$1503**, and its page for
...
**$1A9A** / play **$1A4F**, different again) — five different value sets
across five files of this same merged family (and different again from
...
anything, the spread across five different addresses (even two within the
...
The RWE, Cyb2, TTWII, and Bantam alias merges are all identity/provenance-
level research (tag signature absence + authorship/date evidence), not a
runtime verification — none changes the card's `status`.
...
28085), and "Ninja Remix" (id 990), the VGMPF Future Composer wiki article,
the `rockmonitor.md` merge precedent, and the sibling
`future-composer.md`/`mon-deenen.md` cards this one cross-references.
