...
```json
{
  ...
  "quirks": [
    ...
    "No CSDb release entry exists for MSSIAH (csdb_id: 0 in DeepSID players.json, no dedicated csdb.dk release page found, and confirmed again in the 2026-07-31 re-research pass below) — consistent with it being commercial retail hardware rather than a scene release, and with a CSDb moderator's own statement that it 'is not really scene related'.",
    ...
    "The 'Mssiah' Player-ID tag cannot be pinned to one specific onboard component (Sequencer vs Bassline vs Mono Synth vs the other MIDI apps) — DeepSID players.json and mssiah.com/resources.php describe the cartridge as a suite of five applications sharing one PRG/SID export tool, and no source or format doc distinguishes their export output by component. Recorded here as an explicit ambiguity rather than guessed at.",
    "Re-research pass, 2026-07-31 (csdb_release provenance): searched csdb.dk site search for 'mssiah' (https://csdb.dk/search/?seinsel=all&search=mssiah) — found only two unrelated hits (a 2017 music release 'Analoque' and a SID tune 'I Dream About MSSIAH'), no release/group/scener entry for the product itself. Checked the CSDb forum thread on MSSIAH (https://csdb.dk/forums/index.php?roomid=14&topicid=65480), where moderator Mace states outright 'the MSSIAH is not really scene related' and redirects discussion to the MSSIAH Forum/mssiah.com — direct evidence explaining the absence of a CSDb release id. Also searched for a CSDb group page for '8bit Ventures'/'8-Bit Ventures'; none found. Confirms the prior pass's finding that csdb_release is genuinely absent rather than merely unresearched. No Tier 3 field touched in this pass."
  ],
  "sources": [
    ...
    "CSDb site search for 'mssiah': https://csdb.dk/search/?seinsel=all&search=mssiah (re-run 2026-07-31, no release/group entry for the product)",
    "CSDb forum, MSSIAH thread: https://csdb.dk/forums/index.php?roomid=14&topicid=65480 (moderator Mace: 'the MSSIAH is not really scene related')",
