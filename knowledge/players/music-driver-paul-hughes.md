...
```json
{
  ...
  "quirks": [
    ...
    "SHARED OCEAN COMPOSER COHORT with [[martin-galway]] — surfaced 2026-07-17 by a composer-overlap connection scan over data/composers/*.json. Three of this driver's composers — Jonathan Dunn, Matthew Cannon, Peter Clarke (all English/Ocean-era) — also have files fingerprinted under the [[martin-galway]] tag (Galway's own Ocean in-game driver). This dovetails with the documented history above: Hughes wrote this driver AFTER Galway left Ocean, expressly as a replacement 'for the new guys to use', so the same Ocean composer circle spans both routines. It is an organizational/personnel succession at Ocean plus a shared-composer link, NOT code lineage — Hughes wrote his driver from scratch, not from Galway's code — so no `successor_of` or `shares_routine_with` edge is asserted; navigational link only."
  ],
  "sources": [
