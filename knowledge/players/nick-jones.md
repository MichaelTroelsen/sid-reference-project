...
```json
{
  ...
  "quirks": [
    ...
    "csdb_release IS CONFIRMED NULL, NOT MERELY UNSEARCHED: re-queried CSDb webservice type=sid depth=3 for all three tagged ids (15911/15912/15913) — none carries a release-type entry for the routine/tool itself, only `UsedIn` entries for unrelated demo/game releases that reused the .sid (Equinox->SWT Mix Nr.3 id 100787 etc.; Exolon->Zynalgrexurls id 37850 etc.; Frostbyte->none at all). There is no CSDb release id to record because this was never released as a tool — consistent with the personal-in-game-routine finding above."
  ],
  "sources": [
