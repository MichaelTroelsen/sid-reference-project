...
```json
{
  ...
  "quirks": [
    ...
    "data/players.json's 'encoding' field reads 'PAL + NTSC (PAL frequency table only)' — i.e. the player runs on both PAL and NTSC machines but has no separate NTSC note-frequency table, implying pitch is not corrected for the NTSC clock rate. Its 'aux_support' field reads 'Filter external audio input', meaning the SID's filter-external-input path is used (a hardware curiosity, not confirmed by disassembly). Both are DeepSID's own curated spec fields, not independently verified here.",
    "Re-checked this round: full census of data/composers/*.json for the exact tag 'Novaload' (glob *.json across all composer files) reproduces exactly the same 10 files / 4 composers already on file (Gary Sabin x3, Jeff Minter x1, Aviador Dro x5, Mark Cooksey x1) — no alias variants (e.g. 'Novaload!', version-numbered tags) exist in the dataset. CSDb webservice queries this round independently reconfirm both cited ids: release 133037 = 'Novaload Construction Kit V2.2' (C64 Tool, 1984), and its UsedSIDs entry is SID id 38789 ('Novaload (loader)', load/init 4096 / play 4099 / 1124 bytes) — the two ids sit in their correct, distinct CSDb namespaces (release vs sid). A fresh Lemon64 search (thread https://www.lemon64.com/forum/viewtopic.php?t=13819) surfaced scener recollections of multiple later Novaload tape-loader variants (one nicknamed 'Supernova' by a forum poster) but nothing concrete enough to cite as a distinct player/edge — not added."
  ],
  "sources": [
    ...
    "Re-verified this round via scripts/lib/csdb-client.js (XML webservice, not HTML) directly against csdb.dk: getRelease(133037) and cross-checked its UsedSIDs entry against SID id 38789 — both ids confirmed live and in their correct namespaces",
    "Lemon64 thread 'Novaload Explode' (scener recollections of multiple later Novaload tape-loader variants, none concrete enough to cite as fact): https://www.lemon64.com/forum/viewtopic.php?t=13819",
    "Forum64.de search for 'novaload' this round found only a tape-to-wav conversion tool thread, no additional provenance: https://forum64.de/index.php?thread%2F9695-novaload2wav="
  ]
}
```
