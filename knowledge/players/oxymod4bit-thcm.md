...
```json
{
  ...
  "released": "2008-2025 across all 24 locally-tagged files (both tags), per each file's own CSDb SID-entry 'Released' field (not UsedIn), queried via csdb.dk/webservice type=sid for the full census this session. Earliest: THCM's own 'Hunter's Moon' / 'Rawsynth' / 'Total Triple Trouble' / 'Tristar Boulder' / 'Vicious SID (part 2)', all '2008 The Human Code Machine' (csdb sid ids 39886/39889/39887/39890/39888). Latest: Magnar's 'Unit5', '2025 Magnar Harestad' (csdb sid id 64561), and Mahoney's 'Stocketitt's Grand Piano', '2025 Mahoney' (csdb sid id 64560). No distinct tool-release date exists (never publicly released, see quirks); earliest attested USE predates even THCM's own 15 Nov 2008 beta-tester forum announcement, since the Vicious Sid demo containing four of these five earliest files premiered 29 Oct 2008 at X'2008 (per that file's CSDb UsedIn.Release data).",
  "status": "stub",
  ...
  "quirks": [
    ...
    "Re-research pass, 2026-07-31 (Tier 1/2 gap-fill: `released`, `csdb_release`). CENSUS (not sample): queried csdb.dk's webservice (type=sid) for all 24 locally-tagged files' own CSDb SID entries — 24/24 resolved, no errors. Full per-file year/csdb_id list (composer, name, tag, csdb sid id, Released field): Acrouzet 'Extensions' OxyMod4Bit 63429 '2024 Genesis Project'; Adam Morton 'Break5' OxyMod 56760 '2019 Onslaught'; Adam Morton 'Ravish3' OxyMod4Bit 62404 '2023 Onslaught'; HJE 'Vicious SID 2 - Turn Disk' OxyMod 47334 '2012 The Noisy Bunch'; Jan Albartus 'RagBeat64' OxyMod 42043 '2009 Logan'; LMan 'Make Noise' OxyMod4Bit 61942 '2023 Performers'; Magnar 'Comaland (end)' OxyMod4Bit 58862 '2014 Censor Design/Oxyron'; Magnar 'Comaland 100% (end)' OxyMod4Bit 58864 '2015 Censor Design/Oxyron' (this is the LATER, separate 2015 Comaland 100% production, not the 2014 Comaland demo — consistent with the existing Overview note); Magnar 'Unit5' OxyMod4Bit 64561 '2025 Magnar Harestad'; Mahoney 'Eat It' OxyMod4Bit 51621 '2015 Mahoney'; Mahoney 'Kapla Caves' OxyMod4Bit 50161 '2014 Mahoney'; Mahoney 'Storebror' OxyMod 45693 '2011 Mahoney'; Mahoney 'Stocketitt's Grand Piano' OxyMod 64560 '2025 Mahoney'; Slaxx 'Popcorn' OxyMod4Bit 61221 '2022 Hokuto Force'; THCM 'Hunter's Moon' OxyMod 39886 '2008 The Human Code Machine'; THCM 'Oxy Rock (tune 2)' OxyMod4Bit 50444 '2014 Oxyron'; THCM 'Oxy Rock (tune 3)' OxyMod4Bit 50445 '2014 Oxyron'; THCM 'Rawsynth' OxyMod 39889 '2008 The Human Code Machine'; THCM 'Total Triple Trouble' OxyMod 39887 '2008 The Human Code Machine'; THCM 'Tristar Boulder' OxyMod 39890 '2008 The Human Code Machine'; THCM 'Vicious SID 2 - Blood Money' OxyMod4Bit 48081 '2012 The Noisy Bunch'; THCM 'Vicious SID (part 2)' OxyMod4Bit 39888 '2008 The Human Code Machine'; Vincenzo 'Liquid Venus' OxyMod4Bit 61046 '2022 Lethargy/Singular Crew'; Wacek 'Do It Fast (Stary Piernik Mix)' OxyMod 46989 '2012 Arise'. This corrects/expands the prior 'observed 2008-2014' note — the actual span is 2008-2025, with the tool apparently still in occasional use 17 years after its earliest attested output. CSDB_RELEASE re-confirmed null: beyond the prior session's zero-result search for 'OxyMod' (still cited below), this session additionally searched CSDb site-search for 'THCMod', 'OxyMod4Bit', and 'MDG-Mod-Converter' — all three return CSDb's standard 'no results, check your spelling / category / submit it' page across every category (releases/groups/sceners/events/BBS/SIDs/forum/comments), confirming no CSDb release/tool entry exists under any known name for this converter. Two additional CSDb forum threads found (not previously cited) corroborating continued scene relevance: topicid=168462 (Nov 2024, room 14) — Narciso: \"Thcmod generates a peculiar SID, with its own irq routine, so you don't have to make irq calls\"; Dano: \"A THCMod is not just a simple music called once per frame. It's using custom code to play the music AND all the needed sample work\" — https://csdb.dk/forums/?roomid=14&topicid=168462 ; and topicid=158253 (Nov 2022, room 9), a passing reference to \"very latest THCMod tech\" with no additional provenance beyond confirming the name's continued currency in scene talk — https://csdb.dk/forums/?roomid=9&topicid=158253. Both are third-party forum descriptions of runtime structure (custom IRQ routine, not a simple per-frame call) — noted here for a future disassembly pass, but deliberately NOT encoded into `entry`/`speed`, which stay TODO per this card's Tier 3 boundary. csdb.dk's HTML site 503'd intermittently throughout this session (search and forum pages both, recovering within 1-3 retries); the webservice (`?type=sid`) used for the 24-file census did not 503 once. Aside: the same webservice SID entries also expose LoadAddr/InitAddr (PSID header values, e.g. Tristar Boulder: LoadAddr 2061/InitAddr 2061, i.e. $080D) — PSID container metadata, not disassembly-derived facts, and deliberately NOT written into this card's `memory`/`entry` fields, which stay TODO per the Tier 3 boundary."
  ],
  "sources": [
    ...
    "CSDb webservice type=sid, 24-file census (2026-07-31 re-research pass): queried https://csdb.dk/webservice/?type=sid&id=<id> for each of the 24 locally-tagged files' own csdb_id (from data/composers/*.json) — see the 'Re-research pass' quirks entry for the full per-file id/year table. Used to fill `released`.",
    "CSDb site-search zero-result re-confirmation (2026-07-31): https://csdb.dk/search/?search=THCMod , https://csdb.dk/search/?search=OxyMod4Bit , https://csdb.dk/search/?search=MDG-Mod-Converter — all three return CSDb's standard no-match page, no release/scener/group/SID/forum/comment entry under any of these names.",
    "CSDb forum thread 168462 (room 14, Nov 2024): Narciso and Dano describe ThcMOD's custom IRQ-driven playback structure — https://csdb.dk/forums/?roomid=14&topicid=168462",
    "CSDb forum thread 158253 (room 9, Nov 2022): passing reference to 'very latest THCMod tech', confirms continued scene currency of the name — https://csdb.dk/forums/?roomid=9&topicid=158253"
  ]
}
```
...
rather than a mass-distributed tool. A full CSDb census of all 24 files' own
SID-entry `Released` fields (2026-07-31 pass) puts the tool's attested usage
span at 2008 (THCM's own tracks, predating even his November 2008 forum
beta-tester call) through 2025 (Magnar's "Unit5", Mahoney's "Stocketitt's
Grand Piano") — 17 years of intermittent use, corroborated by a November 2024
CSDb forum thread still discussing ThcMOD's custom IRQ-driven playback
structure.
