...
```json
{
  ...
  "quirks": [
    ...
    "A third research pass re-queried CSDb's own XML webservice directly (scripts/lib/csdb-client.js getRelease(122335)) rather than the HTML page, and it independently reproduces every fact already in this card (Playstar Editor V2.0, 1996, code/music/design/idea: Rabbi (handle ID 6802, Poland), graphics: Condic) plus one new data point: the release's UsedSIDs entry for 'Erotic Malfunction' (csdb_id 24689) carries PSID header LoadAddr=19200 ($4B00), InitAddr=19200 ($4B00, i.e. init==load), PlayAddr=19264 ($4B40), SIDModel 8580, PAL. This is PSID header metadata read off CSDb's catalogue, not a disassembly fact — it is recorded here only, never in the Tier 3 `entry`/`memory` fields above, per the extraction template's rule distinguishing header metadata from reverse-engineered facts. A confirmatory WebSearch for 'Rabbi PlayStar Commodore 64 Andrzejewicz csdb' also turned up nothing beyond generic CSDb-site results, consistent with the prior passes' conclusion that no independent writeup exists."
  ],
  "sources": [
    ...
    "CSDb XML webservice, release 122335, queried directly via scripts/lib/csdb-client.js getRelease() on 2026-08-07 — reconfirms credits/year and adds PSID header fields (LoadAddr/InitAddr/PlayAddr/SIDModel/ClockSpeed) for the UsedSIDs entry 'Erotic Malfunction' (csdb_id 24689)"
  ]
}
```
