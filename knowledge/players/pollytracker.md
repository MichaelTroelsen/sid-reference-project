...
```json
{
  ...
  "quirks": [
    ...
    "A round-29 lead suggested PollyTracker might actually be a modern (post-2010s) open-source tool by 'Kenneth Cepeda (LMan)' — investigated and REFUTED: web search on 'Kenneth Cepeda LMan C64 tracker' and 'PollyTracker C64 tracker open source' turns up only Aleksi Eeben's 2005 tool (CSDb 18605, Demozoo 101055) plus an unrelated C64 musician 'LMan' (CSDb release 193111, 'SID Chip Club', 2020) who has no documented connection to any tracker named PollyTracker. Discarding the lead rather than repeating it.",
    "Author's own project page (referenced in a Lemon64 thread as http://www.kolumbus.fi/aleksieeben/pollytracker/) is dead — kolumbus.fi (a Finnish ISP's defunct personal-homepage host) refuses the connection as of 2026-08-07 — so it cannot be checked for source code either; the CSDb/archive.org binary releases remain the only surviving distribution."
  ],
  "sources": [
    ...
    "Local dataset: 63 files tagged PollyTracker across 25 composers, re-confirmed 2026-07-23 by grepping data/composers/*.json directly (knowledge/COVERAGE.md no longer lists this family individually now that 100% of families are carded); re-censused again 2026-08-07 with an explicit glob against every file in data/composers/*.json — exact same result, 63/25, no correction needed",
    "'Kenneth Cepeda (LMan)' open-source-modern-tracker lead (2026-08-07): searched and found no such tool; see quirks entry above — discarded, do not re-chase"
  ]
}
```
