...
```json
{
  ...
  "quirks": [
    ...
    "PSID header metadata (not a disassembly fact — kept here per this project's own rule) for the sole tagged file, queried directly from CSDb's XML webservice (type=sid, id=39914): load=$5F7C (24444), init=$5F7C (24444), play=$6248 (25192), SIDModel 6581, PAL, 1 subtune, 20835 bytes. The same webservice record independently lists the tune's own 'Released' field as \"2006 Vintaque\" — a second, independent data point (distinct from SIDId's cartridge-release comment) that happens to land on the same 2006 year already used for `released`, mildly corroborating it without resolving the c64-wiki 2004/2006 ambiguity discussed above (a single tune's release year is not proof of the hardware's ship date).",
    "CSDb webservice (type=release, id=64286) confirms the crack release's own metadata directly (not just via its HTML page): Name \"Prophet64\", AKA \"Prophet 64\", Type \"C64 Crack\", released 2008-03-24 at Breakpoint 2008 by Dicke Eier Weihnachtsfeier, Website http://www.prophet64.com — consistent with, and slightly more precise than, the HTML-page reading already cited. Still not used as this card's `csdb_release` (it identifies the crack, not the commercial product)."
  ],
  "sources": [
    ...
    "Local dataset: 1 file tagged Prophet64 (data/composers/vintaque.json) — confirmed by direct read of vintaque.json line 118-127 in this pass (collection_path .../MUSICIANS/V/Vintaque/Razorberries.sid, hash e7fa9f6eb460e2d3d8b402dc62780963, csdb_id 39914); knowledge/COVERAGE.md rank 177 as recorded when this card was first written (the current COVERAGE.md snapshot in this pass no longer lists a Prophet64 row at all, i.e. this remains a 1-file family, just below whatever cutoff COVERAGE.md's current listing applies)",
    "CSDb webservice queried directly in this pass: type=sid id=39914 (the one tagged file) and type=release id=64286 (the crack) — see the two entries above for what each returned"
  ]
}
```
