...
```json
{
  ...
  "quirks": [
    ...
    "V0.7 (not in this batch, uncarded) is a candidate to card separately with its own full SIDId record; if/when it is, this card's edges should be revisited for a possible successor_of/derives_from relationship -- not asserted here without direct version-history evidence linking the two tools.",
