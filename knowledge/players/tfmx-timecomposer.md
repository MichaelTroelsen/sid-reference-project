...
```json
{
  ...
  "quirks": [
    ...
    "Round 31 re-verification (2026-08-07): the crack-vs-original `csdb_release` fix from an earlier round was checked and confirmed still correct — the JSON field is the bare integer 199016 (not the crack id 62502, not a URL string). Independently re-queried both release ids directly through scripts/lib/csdb-client.js's XML webservice (type=release, preferred over the HTML site per project convention): id=199016 returns Type 'C64 Tool', ReleasedBy.Group 'Warriors of Time' (id 663), and Credits Code=Rhodan (Handle id 6980) / Music=Matthias Steffens / Graphics=Phil — exactly matching what the card already stated. Id=62502 independently confirmed as Type 'C64 Crack', ReleasedBy.Handle 'Playboy', sole credit CreditType 'Crack' — genuinely just a crack, not an alternate original. New detail: 62502 (the crack) has three working DownloadLinks (a .t64 and two .d64 images); 199016 (the real tool) has none — its own CSDb page cannot serve the tool itself, only point at the crack, plausibly explaining why SIDId's reference field ended up on the crack in the first place.",
    "PSID header metadata (not a disassembly fact, recorded per EXTRACTION-TEMPLATE.md guidance that PSID load/init/play addresses belong in quirks, never Tier 3 fields, until independently confirmed by disassembly): CSDb's UsedSIDs list for release id=199016 names the two Matthias Steffens tracks bundled with the original tool — Passion.sid (LoadAddr 4096 / InitAddr 4096 / PlayAddr 4099) and Timemaniac.sid (LoadAddr 8192 / InitAddr 8195 / PlayAddr 8192, ClockSpeed PAL). The two rips load at different addresses from each other, so no single fixed load address can be inferred from this pair alone — left open for a future Tier 3 pass.",
    "CSDb scener id=6980 (Rhodan) re-queried directly: Handle 'Rhodan', Country Germany, MemberOf Warriors of Time (id 663, status 'ex') and Defence (id 2275, 1989-1990, status 'ex'), Founded Warriors of Time (group id 663) — matches every fact already in this card's sources array; still no real-name field on the CSDb record itself, so 'Oliver Hoeller' remains sourced to SIDId alone. Namespace trap for future passes: CSDb's type=scener webservice endpoint takes a Handle ID, not the internal nested Scener.ID returned inside a Credit/Handle block — querying the nested Scener.ID (6419 here) directly returns an unrelated person ('Ranger', Sweden), a trap worth flagging alongside the project's existing csdb_id/csdb_release warning."
  ],
  "sources": [
    ...
    "CSDb XML webservice (scripts/lib/csdb-client.js), type=release id=199016, depth=2, queried directly 2026-08-07 (round 31): confirms Type 'C64 Tool', ReleaseYear 1990, ReleasedBy.Group 'Warriors of Time' (id 663), Credits Code=Rhodan(6980)/Music=Matthias Steffens(25607)/Graphics=Phil(16763), UsedSIDs = Passion.sid + Timemaniac.sid both by Matthias Steffens — independently reproduces every fact already cited from the HTML page",
    "CSDb XML webservice, type=release id=62502, depth=2, queried directly 2026-08-07: confirms Type 'C64 Crack', ReleasedBy.Handle 'Playboy', sole Credit CreditType='Crack', and 3 working DownloadLinks (199016 has none) — independently confirms the crack-vs-tool distinction already in this card",
    "CSDb XML webservice, type=scener id=6980, depth=2, queried directly 2026-08-07: Handle 'Rhodan', Country Germany, MemberOf/Founded Warriors of Time (663), ex-Defence (2275) — matches the previously-cited scener page; no real-name field present"
  ]
}
```
...
A third pass (round 31, 2026-08-07) re-verified the crack-vs-original
`csdb_release` fix from an earlier round is still correct, and re-queried
both CSDb release ids (199016, 62502) and Rhodan's scener record directly
through `scripts/lib/csdb-client.js`'s XML webservice rather than trusting
the prior WebFetch-based research. All previously recorded facts checked
out exactly; the only new detail is that the crack release (62502) still
carries three working download links while the original tool release
(199016) carries none — a plausible explanation for why SIDId's fingerprint
ended up pointing at the crack. See the two new `quirks` entries.
