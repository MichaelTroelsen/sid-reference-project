...
```json
{
  ...
  "quirks": [
    ...
    "Re-research pass, 2026-07-31 (targeted at `csdb_release`, which is `null`): confirmed there is no dedicated CSDb release page for Hülsbeck's own C64 TFMX driver/editor, consistent with it never having been publicly released. Checked: (1) SIDId's own sidid.json entry for the bare `TFMX` tag — no `reference` field (unlike its two demoscene-editor sibling tags, which both carry one); (2) web search `site:csdb.dk TFMX Editor C64` and `csdb.dk TFMX Hülsbeck C64 driver release` — no CSDb page for a 'TFMX Editor' or 'TFMX driver' tool turned up, only unrelated hits (a 1996 tool called 'TFX', generic char/sprite editors); (3) the games that carry the driver ARE individually cataloged on CSDb as their own game releases (e.g. Garrison, csdb.dk/release/?id=50059) — but a per-game release id is not a release id for the player/tool itself, and mapping this card's `csdb_release` to one specific game's id would misrepresent the field (the driver appears in Starball, Garrison, R-Type, Circus Attractions, Grand Monster Slam, Bugbomber — no single game 'is' the tool). `csdb_release` is therefore correctly left `null`, not a research gap — there is nothing to find because nothing was ever released as a standalone entry."
  ],
  "sources": [
    ...
    "CSDb release (Garrison, first game to publicly carry the driver): https://csdb.dk/release/?id=50059 — cited only to show the driver's usage is catalogued per-game on CSDb, NOT as this card's csdb_release (which stays null — no standalone tool/driver release exists)",
    "Re-research pass, 2026-07-31: web searches 'site:csdb.dk TFMX Editor C64' and 'csdb.dk TFMX Hülsbeck C64 driver release' found no dedicated CSDb release page for the driver/editor itself, confirming csdb_release should remain null rather than being a research gap"
  ]
}
```
