...
```json
{
  ...
  "quirks": [
    "The leading '?' is DeepSID/SIDId's own low-confidence tag-match marker, the same convention documented on other cards in this KB (e.g. knowledge/players/paul-hodgson.md, knowledge/players/msb.md, knowledge/players/hoernell-players.md). Here it is doubly hedged: DeepSID's php/pretty_player_names.php mapped '?TheSpy_Digi' => \"The Spy's digi player/?\" — note the trailing '/?', DeepSID's own uncertainty about the attribution, independent of the tag-prefix convention, and the source's own inline comment '// Need more info' on that exact line. Verified at the last commit before the file was removed: https://raw.githubusercontent.com/Chordian/deepsid/7b5fb9b57737e4bdecc98f06bc47c6db15f2ee5c/php/pretty_player_names.php (line 615). CITATION-ROT NOTE (found in this round's re-verification): commit https://github.com/Chordian/deepsid/commit/27f057014552bbc35df15b55e4e9afa2f3691b6e ('Tweaks to player names') removed php/pretty_player_names.php entirely and replaced the hardcoded name map with a DB-driven lookup (`SELECT pretty_name FROM players_pretty WHERE raw_name = ...` in php/music.php) — the file no longer exists on master, so the previously-cited master-branch URL now 404s. The fact itself is unchanged (confirmed via the historical commit above); only the citable location moved.",
  ...
  "sources": [
    ...
    "DeepSID display-name mapping (\"The Spy's digi player/?\"), read at the last commit before the file's removal: https://raw.githubusercontent.com/Chordian/deepsid/7b5fb9b57737e4bdecc98f06bc47c6db15f2ee5c/php/pretty_player_names.php (line 615). The file was removed from master by commit https://github.com/Chordian/deepsid/commit/27f057014552bbc35df15b55e4e9afa2f3691b6e (2026-07-28, \"Tweaks to player names\"), which moved the mapping into a `players_pretty` DB table read from php/music.php — the old master-branch URL now 404s; re-verified 2026-08 that the fact still holds via the historical commit.",
