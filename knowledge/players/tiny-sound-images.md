...
```json
{
  ...
  "released": "TODO: no single release date — a proprietary in-house driver used across UK commercial game conversions. Census of all 16 tagged files' own CSDb `<Released>` fields (2026-07-31, every file checked directly via the CSDb webservice, not sampled) shows the games themselves carry commercial release dates spanning 1988 (Bob's Full House, Domark) to 1991 (Judge Dredd, Virgin Mastertronic; Pang!, Ocean) — narrower and more precisely sourced than an earlier 'c. 1988-1994' estimate. These are each game's own genuine release date (from CSDb, not a title-year guess), not a driver version-release date; the driver itself was never separately released or dated. Williams' own 2011 c64.com interview confirms he wrote sound drivers as a matter of course at every studio he worked at (Icon Design, Software Creations, Lothlorien, then Sound Images), so the driver's origin likely predates the 'Sound Images' company name itself (founded 1989 per VGMPF) — not independently confirmed which studio's driver the SIDId tag actually captures.",
  "status": "stub",
  ...
  "quirks": [
    ...
    "Re-research pass, 2026-07-31 (focused on `released`/`csdb_release` only, no Tier 3 work): censused all 16 tagged files' own CSDb SID-entry `<Released>` field directly via the CSDb webservice (type=sid, one query per id, every file checked — not a subset). Tony Williams (9 files, all player=Tiny/Sound_Images; his other 2 dataset files, Sector 90 and Star Wars Droids, are tagged player=David_Whittaker and excluded): Bob's Full House id 31406 = 1988 Domark; Great Courts id 31407 = 1990 Blue Byte/Ubi Soft; Judge Dredd id 46373 = 1991 Virgin Mastertronic; Kendo Warrior id 31408 = 1989 Byte Back; Micro Mouse id 31409 = 1989 Mastertronic; Ninja Warriors id 31410 = 1989 The Sales Curve/Virgin; Octoplex id 31411 = 1989 Mastertronic; Psycho Hopper id 31412 = 1989 Mastertronic (credited to Williams & Tonge jointly); Shinobi id 31414 = 1989 Virgin Games/Sega. Paul Tonge (7 files, all Tiny/Sound_Images): Cosmic Pirate id 1285 = 1989 Palace; Demons Kiss id 29006 = 1989 MC Lothlorien; N.A.R.C. id 29007 = 1990 Ocean; Pang! id 29008 = 1991 Ocean; Pro Mountain Bike Simulator id 29009 = 1990 Zeppelin Games; Saint Dragon id 29010 = 1990 Sales Curve; Xenon id 29011 = 1989 Melbourne House. Full census range: 1988 (earliest, Bob's Full House) to 1991 (latest, Judge Dredd and Pang!, tied) — narrower than the prior 'c. 1988-1994' estimate, which cited only Judge Dredd (1991) and NARC/Turtles (misattributed 1990-91 — NARC is actually 1990, and 'Turtles' does not appear among these 16 CSDb entries at all under this tag; Williams' own interview lists a 'Teenage Mutant Ninja Turtles' credit but no such file carries this Player-ID tag in this dataset). These are genuine per-game commercial release dates (CSDb's own `<Released>` field, not a title-year guess), not a driver version date — the driver itself carries no separate release. `csdb_release` re-confirmed null this pass: attempted to re-run the CSDb HTML site search for 'Sound Images' (https://csdb.dk/search/?search=Sound+Images&type=all) but the HTML site returned HTTP 503 on two attempts (site known to 503 intermittently, per project method notes) — could not independently re-verify via the search UI this pass, so this pass relies on the prior pass's confirmed-empty result (still cited below) rather than asserting a fresh negative. None of the 16 SID entries' CSDb pages surfaced a separate driver/tool release — each entry's only release association is the commercial game itself, already tabulated above."
  ],
  "sources": [
    ...
    "Re-research pass 2026-07-31: CSDb webservice, one type=sid query per tagged file (all 16, census not sample), used to redate `released` to a 1988-1991 range: id=31406 Bob's Full House (1988), id=31407 Great Courts (1990), id=46373 Judge Dredd (1991), id=31408 Kendo Warrior (1989), id=31409 Micro Mouse (1989), id=31410 Ninja Warriors (1989), id=31411 Octoplex (1989), id=31412 Psycho Hopper (1989), id=31414 Shinobi (1989), id=1285 Cosmic Pirate (1989), id=29006 Demons Kiss (1989), id=29007 N.A.R.C. (1990), id=29008 Pang! (1991), id=29009 Pro Mountain Bike Simulator (1990), id=29010 Saint Dragon (1990), id=29011 Xenon (1989) — e.g. https://csdb.dk/webservice/?type=sid&id=31406&depth=1",
    "Re-research pass 2026-07-31: attempted re-check of CSDb HTML search for 'Sound Images' as a group/release name — site returned HTTP 503 on two attempts (could not re-verify independently this pass; the empty result cited above is from the prior pass, not this one): https://csdb.dk/search/?search=Sound+Images&type=all"
  ]
}
```
...
template describes. A full census of all 16 tagged files' own CSDb
`<Released>` fields (2026-07-31) puts the games themselves at 1988
(Bob's Full House) to 1991 (Judge Dredd, Pang!) — the games' commercial
release dates, not a date for the driver, which was never separately
released.
