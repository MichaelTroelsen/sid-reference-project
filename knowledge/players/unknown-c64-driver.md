...
```json
{
  ...
  "quirks": [
    ...
    "VGMPF's driver-page game table lists Sigue Sigue Sputnik's release year as '2015-12-21' ('According to GTW64'), which is almost certainly the year of a much later re-release/tribute version of the game rather than a real 1980s date for this driver — the driver's own documented first-use date on the same page is '1986-0?-??' (Empire). This is reinforced, not just guessed: CSDb's own SID-entry for the file (id=53045, queried directly via the webservice 2026-07-31) gives its own `Released` field as '1986 Significance', not 2015 — i.e. the underlying music/SID release itself is contemporaneous with the driver's other 1986-1989 games, and only VGMPF's *game* re-release date is the outlier. Flagged here as a probable VGMPF table anomaly (conflating the tune's original release with a much later game reissue), not treated as evidence the driver was still in active development in 2015.",
    "csdb_release gap re-checked (2026-07-31, full census of all 4 tagged files): queried CSDb's webservice (type=sid) directly for each of the 4 local '?Source'-tagged files' own `Released` fields — Sigue Sigue Sputnik id=53045 '1986 Significance', Street Beat id=1917 '1987 Mastertronic', Psycho Soldier id=1768 '1987 Imagine', Rainbow Warrior id=13318 '1989 Micro Style'. All four predate or match VGMPF's stated ~Feb 1988 supersession by the successor driver, consistent with the sibling card's finding that no '?Source_2'-tagged file predates that date. Separately searched csdb.dk's HTML search (https://csdb.dk/search/?search=Unknown+C64+Driver&type=all) for a dedicated tool/release/group page for this driver: zero results ('We were unable to find anything for you'). Lemon64's forum search requires login (blocked, could not query); Forum64.de returned HTTP 403 to an unauthenticated request. No dedicated CSDb release id exists for this driver as a standalone tool on the evidence available, so `csdb_release` stays `null` — a confirmed negative from the CSDb side, and an inconclusive (not negative) result from Lemon64/Forum64 since both blocked read access rather than returning a clean no-match. No community thread was posted to ask; per instructions this project's research stays read-only.",
  ...
  "sources": [
    ...
    "CSDb — full census (all 4, via the webservice type=sid, queried directly 2026-07-31) of local '?Source'-tagged files' own `Released` fields: Sigue Sigue Sputnik id=53045 '1986 Significance' https://csdb.dk/sid/?id=53045, Street Beat id=1917 '1987 Mastertronic' https://csdb.dk/sid/?id=1917, Psycho Soldier id=1768 '1987 Imagine' https://csdb.dk/sid/?id=1768, Rainbow Warrior id=13318 '1989 Micro Style' https://csdb.dk/sid/?id=13318",
    "csdb.dk HTML search (https://csdb.dk/search/?search=Unknown+C64+Driver&type=all), checked 2026-07-31: zero results for a dedicated tool/release/group page for this driver",
    "Lemon64 (lemon64.com) forum search and Forum64.de (forum64.de) searched by name for 'Unknown C64 Driver', checked 2026-07-31: Lemon64's forum search requires a login (blocked); Forum64.de returned HTTP 403 to an unauthenticated request — both inconclusive, not a confirmed no-match",
...
found. A 2026-07-31 gap-filling pass re-checked `csdb_release` specifically:
a full census (all 4 files, not a sample) of their own CSDb SID-entry
`Released` fields was pulled directly from the webservice, and csdb.dk's
HTML search was queried by name for a dedicated tool page — zero results.
Lemon64 and Forum64.de were also queried by name but both blocked
unauthenticated/JS-less access, so that half of the check is inconclusive
rather than a clean negative; `csdb_release` stays `null`. Every Tier 3
runtime field is honestly `TODO`.
