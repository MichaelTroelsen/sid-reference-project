...
```json
{
  ...
  "quirks": [
    ...
    "Re-verified via CSDb webservice (scripts/lib/csdb-client.js getRelease(12580)) that release 12580's `Type` field is 'C64 Tool', not 'C64 Crack' — the SIDId `reference` correctly points at the original tool release, not a crack of it, so the recurring 'reference points at a crack' trap does not apply here.",
    "That same webservice call's `UsedSIDs` list gives PSID header addresses for 8 of the 9 bundled example tunes (all of Walt's own Music_1/2/3/4/6/7/8/Test-Tune plus Link's 'Sanne'): every one loads/inits at $1000 (4096) and plays at $1003 (4099) — a genuinely consistent pattern across 8 independently-compiled tunes from this exact release, unlike the single earlier-sampled outlier (CSDb sid id 30601 'Amiga Works (part 1)', a later 1991 tune NOT part of this release, whose reported load $3E00/init $7A08/play $0000 looks anomalous by comparison). This is still PSID header metadata, not a disassembly fact, so it stays here in quirks rather than being promoted into the Tier 3 `entry`/`memory` fields — but it is a much stronger signal of the engine's real fixed base than the one outlier was."
  ],
  "sources": [
    ...
    "CSDb webservice getRelease(12580) via scripts/lib/csdb-client.js, this pass: Type 'C64 Tool' (rules out the crack-vs-original trap) and UsedSIDs list giving consistent LoadAddr/InitAddr 4096, PlayAddr 4099 across 8 of the 9 bundled example tunes",
    "CSDb sid entry 'Amiga Works (part 1)', Anders Fogh (Walt), 1991 Bonzai (sampled PSID header only): https://csdb.dk/sid/?id=30601",
    "Local dataset: 95 files tagged 'Walt/Bonzai' across 2 composers — Walt 79, Tjagvad 16 (re-censused this pass via grep of data/composers/*.json with an explicit glob; knowledge/COVERAGE.md has since been regenerated to 100% coverage and no longer lists this family by name, so it's cited only as historical context, not as the count's source)",
